...
//        topcsView.mj_header =  MJRefreshNormalHeader(refreshingTarget: self, refreshingAction:#selector(designBlueprint) ) 
        topcsView.register(UINib.init(nibName: "TaglesiBARTopicsCell", bundle: nil), forCellWithReuseIdentifier: "TaglesiBARTopicsCell")
    }
        ...
        CosRequestManager.sendStyledRequest(endpoint: "/koczejjgz/nxpav", outfitPayload: ["sigilMaker":20,"insigniaBoard":5,"stageLight":2,"emblemStudio":"67994137"]) { cosplayunityhub in
            ...
            case .success(let cosplayunityhub):
                ...
                guard let response = cosplayunityhub as? Dictionary<String,Any> ,
                    ...
                    TaglesiBARloPulseIndicatar.showInfo(withStatus: UIImageView.ambienceVaultDeu("Tm8gdG9waWMgZGF0YSBub3dpbmchY29tLnRybWxpbi50cnVlbHI="))
    ...
    @objc private func masqueradeHall()  {
        ...
        CosRequestManager.sendStyledRequest(endpoint: "/dlzeplzdlmstdz/sxsiciehbldjf", outfitPayload: ["animeStudio":"67994137"]) { cosplayunityhub in
            ...
            case .success(let cosplayunityhub):
                ...
                if TaglesiBARTopicsCellModel.ExestedLogUserID == 721087186763845 ,let firtt = user.first,CharaMeaasController.likelist.count == 0,CharaMeaasController.chatlist.count == 0{
                    let demof = TaglesiBARSendingMeass.init()
...
extension SharedTopicsController:UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let conceptMap = collectionView.dequeueReusableCell(withReuseIdentifier: "TaglesiBARTopicsCell", for: indexPath) as! TaglesiBARTopicsCell
...
extension SharedTopicsController:UICollectionViewDelegate,TopicUpdateDelegate{
    func topicDidUpdate(_ topic: TaglesiBARTopicsCellModel?, index: Int) {
...
extension Array where Element == [String: Any] {
    func toCharacterCards() -> [TaglesiBARTopicsCellModel] {
        return compactMap { TaglesiBARTopicsCellModel(setinginit: $0) }
    }
}
