...
class CarnivalParadeController: UIViewController {
    ...
    @IBAction func artistryHub(_ sender: UIButton) {
        ...
        if isFlameActive == true {
            ...
            TaglesiBARloPulseIndicatar.showInfo(withStatus: UIImageView.ambienceVaultDeu("TWF0Y2hpbmcgZmFpbGVkLCB0aGUgb3RoZXIgcGFydHkgaGFzIGxlZnRjb20udHJtbGluLnRydWVscg=="))
            self.navigationController?.popViewController(animated: true)
        }
    ...
    private func igniteFlameTicker() {
            flameTicker = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] _ in
                ...
                if self.remainEchoSeconds <= 0 {
                    ...
                    TaglesiBARloPulseIndicatar.showInfo(withStatus: UIImageView.ambienceVaultDeu("U29ycnksIGl0J3MgdGltZWNvbS50cm1saW4udHJ1ZWxy"))
