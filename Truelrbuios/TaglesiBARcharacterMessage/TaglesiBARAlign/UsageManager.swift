...
class TaglesiBARBuyUsageManager {
    ...
    func canAffordMatch() -> Bool {
        ...
        return TaglesiBARSomiccon.CurrentCoinggUserOwne >= matchCost
    }
    ...
    func deductMatchCost() {
        if remainingFreeMatches > 0 {
            ...
            // Deduct coins
            let newBalance = TaglesiBARSomiccon.CurrentCoinggUserOwne - matchCost
            TaglesiBARSomiccon.CurrentCoinggUserOwne = newBalance
    ...
    func showMatchConfirmation(in viewController: UIViewController) {
        ...
        } else if TaglesiBARSomiccon.CurrentCoinggUserOwne >= matchCost {
    ...
    private func showPaidMatchAlert(in viewController: UIViewController) {
        let alert = UIAlertController(
            ...
            message: "This match will cost 20 coins. Your current balance: \(TaglesiBARSomiccon.CurrentCoinggUserOwne) coins.",
            preferredStyle: .alert
        )
    ...
    func showInsufficientBalanceAlert(in viewController: UIViewController) {
        let alert = UIAlertController(
            ...
            message: "You need 20 coins to start a match. Your current balance: \(TaglesiBARSomiccon.CurrentCoinggUserOwne) coins.",
            preferredStyle: .alert
        )
